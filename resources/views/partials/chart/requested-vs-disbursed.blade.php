<div class="rounded-2xl border border-gray-200 bg-white px-5 pb-5 pt-5 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6 sm:pt-6">
  <div class="flex flex-col gap-5 mb-6 sm:flex-row sm:justify-between">
    <div class="w-full">
      <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">
        Requested vs Disbursed
      </h3>
      <p class="mt-1 text-gray-500 text-theme-sm dark:text-gray-400">
        Comparison of loan amounts requested and disbursed
      </p>
    </div>
  ...
  <div class="max-w-full overflow-x-auto custom-scrollbar">
    <div id="chart-requested-vs-disbursed" class="-ml-4 min-w-[700px] pl-2"></div>
  </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const options = {
        ...
        series: [{
            name: 'Requested',
            data: [stats.total_requested ?? 0]
        },{
            name: 'Disbursed',
            data: [stats.total_disbursed ?? 0]
        }],
        stroke: {
            width: [3, 3],
            curve: 'smooth'
    ...
    const chart = new ApexCharts(document.querySelector("#chart-requested-vs-disbursed"), options);
    chart.render();
});
</script>